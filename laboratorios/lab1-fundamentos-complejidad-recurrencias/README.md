...
**Sobre la propuesta del servidor del doble de velocidad.** El dato medido que la desmiente es directo: en la Parte 4, al pasar de `n = 800` a `n = 1.600` (factor 2×), el tiempo de `insertion sort` pasó de 51,9 ms a 258 ms —un factor 5×, no 2×—. Duplicar la velocidad del reloj divide el tiempo por 2, pero el problema es que el software hace **cuatro veces más trabajo** cada vez que los registros se duplican. La Secretaría volvería a quedarse corta en cuanto el programa crezca otra vez. La inversión se pierde.
