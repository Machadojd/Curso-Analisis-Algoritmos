...
def leer_anios() -> list[int]:
    ...
    while True:
        entrada = input(
            "Ingrese anios separados por comas (ej. 2000,2023,2024): "
        ).strip()
        try:
            fragmentos = [token.strip() for token in entrada.split(",") if token.strip()]
            anios = [int(token) for token in fragmentos]
            return anios
        except ValueError:
            print("Entrada invalida. Asegurese de ingresar solo numeros enteros separados por comas.")
